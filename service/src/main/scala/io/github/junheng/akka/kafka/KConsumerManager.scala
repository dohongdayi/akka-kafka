...
class KConsumerManager(topic: String, group: String, zookeeper: String, log: LoggingAdapter)(implicit execution: ExecutionContext) {
  ...
  private def processPartition(zkClient: ZkClient, group: String, topic: String, pid: Int): Option[PartitionStatus] = {
    ...
    zkUtils = ZkUtils(zkClient, JaasUtils.isZkSecurityEnabled())
    val ownerOpt = zkUtils.readDataMaybeNull(groupDirs.consumerOwnerDir + "/%s".format(pid))._1
  ...
  def getPartitionStatus: Future[List[PartitionStatus]] = Future {
    ...
    try {
      ...
      val result = topicList.sorted.map(topic => processTopic(zkUtils.zkClient, group, topic)).filterNot(_.isEmpty).map(_.get).foldLeft(List[PartitionStatus]())(_ ::: _)
